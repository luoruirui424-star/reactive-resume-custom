...
import { exportAIConfig, importAIConfig } from "@/utils/ai/config-import-export";
...
function AIForm() {
  const { set, model, apiKey, baseURL, provider, enabled, testStatus, exportConfig, importConfig } = useAIStore();
  ...
  const handleExportConfig = () => {
    try {
      ...
      exportAIConfig(config);
      toast.success(t`AI configuration exported successfully`);
    } catch (error) {
  ...
  const handleFileSelect = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      const config = await importAIConfig(file);
      importConfig(config);
      toast.success(t`AI configuration imported successfully. Please test the connection.`);
    } catch (error) {
      toast.error(error instanceof Error ? error.message : t`Failed to import configuration`);
    }
    ...
    <div className="grid gap-6 sm:grid-cols-2">
      ...
      <div className="flex gap-2">
        ...
        <Button variant="outline" onClick={handleExportConfig}>
          <DownloadSimpleIcon />
          <Trans>Export Config</Trans>
        </Button>

        <Button variant="outline" onClick={handleImportConfig}>
          <UploadSimpleIcon />
          <Trans>Import Config</Trans>
        </Button>
      </div>
